...
describe("MembersPage all-branches mode", () => {
    ...
    it("resets to page 1 when active branch changes", async () => {
        const authState = {
            user: { isAdmin: true, globalRole: "ADMIN" },
            activeBranchId: "branch-1",
        };
        ...
        authState.activeBranchId = "branch-2";
        ...
        const lastCall = mockApiClient.mock.calls[mockApiClient.mock.calls.length - 1] as [string, { params: { page: number } }];
        expect(lastCall[0]).toBe("/members");
