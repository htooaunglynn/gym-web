...
// Extracted to be used by silentRefresh in AuthContext directly
export async function silentRefreshApi() {
  const res = await fetch("/api/tenant/auth/refresh", { method: "POST" });
...
export async function apiClient<T>(
  ...
  const url = path.startsWith("http") ? path : `/api/tenant${path.startsWith("/") ? path : `/${path}`}`;
