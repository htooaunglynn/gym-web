...
describe("usePermission", () => {
    ...
    it("returns true when the feature satisfies the required stage", async () => {
        setUser(null);
        setPermissions([
            { feature: "USERS", actions: ["VIEW", "CREATE_UPDATE"], permissionStage: 3 },
        ]);
        const { usePermission } = await import("./usePermission");
        const { result } = renderHook(() => usePermission("USERS", 3));
        expect(result.current).toBe(true);
    });

    it("returns false when the feature stage is below the required stage", async () => {
        setUser(null);
        setPermissions([{ feature: "USERS", actions: ["VIEW"], permissionStage: 1 }]);
        const { usePermission } = await import("./usePermission");
        const { result } = renderHook(() => usePermission("USERS", 4));
        expect(result.current).toBe(false);
    });
