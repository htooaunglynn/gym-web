...
export function Pricing() {
  ...
  useEffect(() => {
    async function fetchActivePlans() {
      try {
        const res = await fetch("http://localhost:3000/api/v1/membership-plans?isActive=true&limit=10");
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 max-w-6xl mx-auto">
          {isLoading ? (
            ...
            plans.map((plan, index) => (
              <div 
                key={plan.id}
                className={`relative group p-8 lg:p-10 rounded-[40px] border transition-all duration-300 ${
                  index === 1 
              ...
              >
                ...
                <div className="flex items-center gap-3 mb-8">
                  <div className={`w-12 h-12 rounded-2xl flex items-center justify-center ${
                    index === 1 ? "bg-brand text-white" : "bg-brand/10 text-brand"
                  }`}>
                ...
                <Link
                  href="/login"
                  className={`w-full py-4 rounded-2xl font-black tracking-widest text-sm transition-all flex items-center justify-center uppercase ${
                    index === 1
