...
export function LoginForm() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        if (res.status === 401) {
          // Req 2.4: inline error below password field
          ...
          setFieldErrors({ general: messages });
        } else {
          setFieldErrors({
            general:
      ...
      setFieldErrors({
        general: "Unable to reach the server. Check your connection.",
      });
    } finally {
      setLoading(false);
    }
          ...
          <div>
            <div className="relative">
              <input
                ...
                className={`w-full px-5 py-3.5 rounded-2xl bg-gray-50 dark:bg-gray-800 text-gray-900 dark:text-white outline-none border transition-colors text-sm placeholder:text-gray-400 ${
